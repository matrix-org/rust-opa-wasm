...
async fn test_policy_with_datafile(
    ...
    let input = match datafile_path {
        Some(path) => {
            let input_bytes = tokio::fs::read(input(path)).await?;
            Some(serde_json::from_slice(&input_bytes[..])?)
        }
        None => None,
    };
...
integration_test!(
    test_loader_false,
    "test-loader",
    input = "test-loader.false.json"
);
integration_test!(
    test_loader_true,
    "test-loader",
    input = "test-loader.true.json"
);
...
async fn test_http() {
    ...
    let content_value: serde_json::Value = json!({ "key": "value" });
    ...
    let get_yaml_mock = server.mock(|when, then| {
        ...
        then.status(200)
            ...
            .body(serde_yml::to_string(&content_value).unwrap());
    });
    ...
    let res = test_policy("test-http", Some(json!({"base_url": server.url("")})))
        .await
        .expect("error in test suite");
    ...
    assert_eq!(
        ...
        &serde_yml::to_string(&content_value).unwrap()
    );
